...
export function CreateCanvasForm({ onCanvasCreated }: CreateCanvasFormProps) {
  ...
  // If confirmed, call the success callback
  if (isConfirmed) {
    onCanvasCreated();
  }
  ...
  return (
    <div className="p-5 bg-white border border-gray-200 rounded-lg shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" id="width" value={width} onChange={e => setWidth(parseInt(e.target.value))} className="w-full p-2 mt-1 border border-gray-300 rounded-md shadow-sm" />
          </div>
          <div>
            <label htmlFor="height" className="block text-sm font-medium text-gray-700">Height</label>
            <input type="number" id="height" value={height} onChange={e => setHeight(parseInt(e.target.value))} className="w-full p-2 mt-1 border border-gray-300 rounded-md shadow-sm" />
          </div>
        </div>

        <div>
          <label htmlFor="mintPrice" className="block text-sm font-medium text-gray-700">Mint Price (ETH)</label>
          <input type="text" id="mintPrice" value={mintPrice} onChange={e => setMintPrice(e.target.value)} className="w-full p-2 mt-1 border border-gray-300 rounded-md shadow-sm" />
        </div>

        <div>
          <label htmlFor="fee" className="block text-sm font-medium text-gray-700">Marketplace Fee (%)</label>
          <input type="text" id="fee" value={fee} onChange={e => setFee(e.target.value)} className="w-full p-2 mt-1 border border-gray-300 rounded-md shadow-sm" />
        </div>

        <button
          type="submit"
          disabled={!isConnected || isLoading}
          className="w-full px-4 py-3 font-bold text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:bg-gray-400"
        >
      ...
      <div className="mt-4 text-xs font-mono">
        ...
        {isConfirming && <div className="text-blue-600">Deploying your canvas...</div>}
        {isConfirmed && <div className="text-green-600">Success! Your canvas is live.</div>}
        {error && (
